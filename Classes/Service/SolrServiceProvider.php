			...
			foreach ($facetConfiguration as $key => $facet) {
				if (array_key_exists('id', $facet)) {
						...
						foreach ($facet['facetQuery'] as $facetQueryIndex => $facetQuery) {
							if (array_key_exists('id', $facetQuery) && array_key_exists('query', $facetQuery)) {
								...
								LoggerUtility::logError(
									sprintf('find: TypoScript facet »%s«, facetQuery %s does not have the required keys »id« and »query«. Ignoring this facetQuery.', $facetID, $facetQueryIndex),
									[
					...
					LoggerUtility::logWarning(
						sprintf('find: TypoScript facet %s does not have the required key »id«. Ignoring this facet.', $key),
						[
							'facet' => $facet,
							'facetConfiguration' => $facetConfiguration
						]
			...
			if (array_key_exists('facetQuery', $facetConfig)) {
				...
				if ($queryString === NULL) {
					LoggerUtility::logInfo(
						sprintf('find: Results for Facet »%s« with facetQuery ID »%s« were requested, but this facetQuery is not configured. Building a generic facet query instead.', $facetConfig['id'], $queryTerm),
						[
		...
		if (is_array($this->settings['sort'])) {
			...
			foreach ($this->settings['sort'] as $sortOptionIndex => $sortOption) {
				if (array_key_exists('id', $sortOption) && array_key_exists('sortCriteria', $sortOption)) {
					...
					$message = sprintf('find: TypoScript sort option »%s« does not have the required keys »id« and »sortCriteria. Ignoring this setting.', $sortOptionIndex);
					LoggerUtility::logWarning(
						$message,
						[
